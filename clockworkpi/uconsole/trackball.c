...
static float rateToVelocityCurve(float input, float acceleration_scale) {
  ...
  // Ramp the base offset smoothly from zero instead of using a hard deadzone.
  // This gives slow movements an immediate response without a sudden jump.
...
report_mouse_t pointing_device_driver_get_report(report_mouse_t mouse_report) {
  ...
  // Cap a stale idle interval so wake-up speed cannot cause cursor overshoot.
  if (delta > 50) {
    delta = 50;
  }
